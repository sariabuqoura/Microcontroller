...
;MyProject.c,34 :: 		unsigned int SPI_Ethernet_UserTCP( char *remoteHost, unsigned int remotePort,unsigned int localPort, unsigned int reqLength)
;MyProject.c,37 :: 		if(localPort != 80)               // I listen only to web request on port 80
	MOVLW       0
...
;MyProject.c,38 :: 		return(0);
	CLRF        R0 
	CLRF        R1 
	GOTO        L_end_SPI_Ethernet_UserTCP
L_SPI_Ethernet_UserTCP0:
;MyProject.c,41 :: 		for(len = 0 ; len < 15 ; len++) getRequest[len] = SPI_Ethernet_getByte();
...
;MyProject.c,42 :: 		getRequest[len] = 0;
	MOVLW       _getRequest+0
	ADDWF       SPI_Ethernet_UserTCP_len_L0+0, 0 
	MOVWF       FSR1 
	MOVLW       hi_addr(_getRequest+0)
	...
	MOVWF       FSR1H 
	CLRF        POSTINC1+0 
;MyProject.c,44 :: 		if(memcmp(getRequest, "GET /", 5)) return(0); // only GET method
	MOVLW       _getRequest+0
...
;MyProject.c,46 :: 		if(!memcmp(getRequest+11, "ON", 2)) // do we have ON command
...
;MyProject.c,47 :: 		PORTB.F0 = 1; // set PORTB bit0
	BSF         PORTB+0, 0 
	GOTO        L_SPI_Ethernet_UserTCP6
L_SPI_Ethernet_UserTCP5:
;MyProject.c,49 :: 		if(!memcmp(getRequest+11, "OFF", 3)) // do we have OFF command
	MOVLW       _getRequest+11
	MOVWF       FARG_memcmp_s1+0 
	MOVLW       hi_addr(_getRequest+11)
	MOVWF       FARG_memcmp_s1+1 
...
;MyProject.c,50 :: 		PORTB.F0 = 0; // clear PORTB bit0
	BCF         PORTB+0, 0 
L_SPI_Ethernet_UserTCP7:
L_SPI_Ethernet_UserTCP6:
;MyProject.c,52 :: 		if (PORTB.F0)
	BTFSS       PORTB+0, 0 
	GOTO        L_SPI_Ethernet_UserTCP8
;MyProject.c,54 :: 		memcpy(indexPage+340, "#FFFF00", 6); // highlight (yellow) ON
	...
	CALL        _memcpy+0, 0
;MyProject.c,55 :: 		memcpy(indexPage+431, "#4974E2", 6); // clear OFF
...
;MyProject.c,59 :: 		memcpy(indexPage+340, "#4974E2", 6); // clear ON
	MOVLW       _indexPage+340
	MOVWF       FARG_memcpy_d1+0 
	MOVLW       hi_addr(_indexPage+340)
	MOVWF       FARG_memcpy_d1+1 
	...
	MOVWF       FARG_memcpy_n+1 
	CALL        _memcpy+0, 0
;MyProject.c,60 :: 		memcpy(indexPage+431, "#FFFF00", 6); // highlight (yellow) OFF
	MOVLW       _indexPage+431
	MOVWF       FARG_memcpy_d1+0 
	MOVLW       hi_addr(_indexPage+431)
	MOVWF       FARG_memcpy_d1+1 
...
;MyProject.c,62 :: 		len = SPI_Ethernet_putConstString(httpHeader); // HTTP header
	MOVLW       _httpHeader+0
...
;MyProject.c,63 :: 		len += SPI_Ethernet_putConstString(httpMimeTypeHTML); // with HTML MIME type
...
;MyProject.c,64 :: 		len += SPI_Ethernet_putString(indexPage); // HTML page fi rst part
	MOVLW       _indexPage+0
	...
	MOVF        R1, 0 
	MOVWF       SPI_Ethernet_UserTCP_len_L0+1 
;MyProject.c,65 :: 		return len; // return to the library with the number of bytes to transmit
;MyProject.c,66 :: 		}
...
;MyProject.c,67 :: 		unsigned int SPI_Ethernet_UserUDP( char *remoteHost, unsigned int remotePort,unsigned int destPort, unsigned int reqLength)
;MyProject.c,69 :: 		return 0; // back to the library with the length of the UDP reply
	CLRF        R0 
	CLRF        R1 
;MyProject.c,70 :: 		}
...
;MyProject.c,71 :: 		void main()
;MyProject.c,73 :: 		ADCON1 |= 0x0F ; // no analog inputs
	MOVLW       15
	IORWF       ADCON1+0, 1 
;MyProject.c,74 :: 		CMCON |= 0x07 ; // turn off comparators
	MOVLW       7
	IORWF       CMCON+0, 1 
;MyProject.c,75 :: 		PORTB.F0 = 0;
	BCF         PORTB+0, 0 
;MyProject.c,76 :: 		TRISB.F0 = 0; // set PORTB.B0 as output (rele control pin)
	BCF         TRISB+0, 0 
;MyProject.c,79 :: 		Spi1_Init();
	CALL        _SPI1_Init+0, 0
;MyProject.c,81 :: 		Spi_Ethernet_Init(myMacAddr,myIpAddr,Spi_Ethernet_FULLDUPLEX);
	MOVLW       _myMacAddr+0
...
;MyProject.c,84 :: 		SPI_Ethernet_confNetwork(ipMask, gwIpAddr, dnsIpAddr);
	MOVLW       _ipMask+0
...
L_main10:
;MyProject.c,87 :: 		SPI_Ethernet_doPacket(); // process incoming Ethernet packets
